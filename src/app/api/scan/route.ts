...
export function heuristicParseOCR(ocrText: string, labels: string[], filename: string) {
  ...
  if (ocrLower.includes("bill") || ocrLower.includes("electric") || ocrLower.includes("power") || ocrLower.includes("kwh")) {
    category = "energy";
    summaryDescription = "Utility Bill (OCR)";
  } else if (ocrLower.includes("receipt") || ocrLower.includes("store") || ocrLower.includes("supermarket")) {
    category = "waste";
  ...
  return {
    ...
    suggestedAlternatives: [
      { name: "Eco alternative", description: "Choose reusable packaging and conserve resource usages." }
    ]
  };
}
