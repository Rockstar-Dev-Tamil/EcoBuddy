...
export default function CommunityPage() {
  ...
  const getAchievementRarity = (xp: number) => {
    if (xp >= 250) {
      return {
        name: "Legendary",
        glowClass: "border-amber-500/30 bg-amber-500/5 hover:shadow-[0_0_25px_rgba(245,158,11,0.2)]",
    ...
    if (xp >= 200) {
      return {
        name: "Epic",
        glowClass: "border-fuchsia-500/30 bg-fuchsia-500/5 hover:shadow-[0_0_25px_rgba(217,70,239,0.2)]",
  ...
  const getGroupTypeColor = (type: string) => {
    switch (type) {
      case "family": return "text-emerald-400 bg-emerald-500/10 border-emerald-500/20";
      case "hostel": return "text-cyan-400 bg-cyan-500/10 border-cyan-500/20";
      case "office": return "text-amber-400 bg-amber-500/10 border-amber-500/20";
      case "college": return "text-indigo-400 bg-indigo-500/10 border-indigo-500/20";
  ...
  const getGroupArchetype = (type: string) => {
    switch (type) {
      case "family": return "Forest Village";
      case "hostel": return "Ecosystem Hub";
      case "office": return "Industrial Jungle";
      case "college": return "Campus Garden";
      default: return "Eco Village";
    }
  };
        ...
        <div className="glass-panel p-6 rounded-2xl border border-white/5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-zinc-950/20">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-400">Compete in leagues, grow villages, and build collectible card decks.</p>
            </div>
          </div>
          ...
          <AnimatePresence mode="wait">
            ...
            {activeTab === "rankings" && (
              ...
              >
                ...
                <div className="grid grid-cols-3 gap-4 items-end max-w-3xl w-full mx-auto py-4">
                  ...
                  {topThree[1] && (
                    <div className="flex flex-col items-center gap-2">
                      <span className="text-[10px] font-extrabold text-zinc-400 uppercase tracking-widest font-mono">Rank 2</span>
                      <div className="w-full h-36 bg-zinc-900/60 border border-zinc-750 p-4 rounded-t-2xl flex flex-col justify-between items-center text-center relative overflow-hidden group hover:border-zinc-500 transition-colors">
                        ...
                        <div className="mt-2">
                          <span className="text-xs font-bold text-white block line-clamp-1">{topThree[1].username}</span>
                          <span className="text-[9px] text-zinc-400 font-mono block mt-0.5">{topThree[1].xp.toLocaleString()} XP</span>
                        </div>
                  ...
                  {topThree[0] && (
                    <div className="flex flex-col items-center gap-2">
                      <span className="text-xs font-extrabold text-yellow-400 uppercase tracking-widest font-mono">Winner</span>
                      <div className="w-full h-44 bg-gradient-to-t from-yellow-500/10 via-zinc-900/80 to-zinc-900 border border-yellow-500/35 p-5 rounded-t-3xl flex flex-col justify-between items-center text-center relative overflow-hidden shadow-[0_-4px_30px_rgba(234,179,8,0.1)] group hover:border-yellow-400 transition-colors">
                        ...
                        <div className="mt-2">
                          <span className="text-xs font-extrabold text-white block line-clamp-1">{topThree[0].username}</span>
                          <span className="text-[9px] text-yellow-400 font-bold font-mono block mt-0.5">{topThree[0].xp.toLocaleString()} XP</span>
                        </div>
                  ...
                  {topThree[2] && (
                    <div className="flex flex-col items-center gap-2">
                      <span className="text-[10px] font-extrabold text-amber-700 uppercase tracking-widest font-mono">Rank 3</span>
                      <div className="w-full h-32 bg-zinc-900/60 border border-zinc-800 p-4 rounded-t-2xl flex flex-col justify-between items-center text-center relative overflow-hidden group hover:border-amber-700/60 transition-colors">
                        ...
                        <div className="mt-2">
                          <span className="text-xs font-bold text-white block line-clamp-1">{topThree[2].username}</span>
                          <span className="text-[9px] text-zinc-500 font-mono block mt-0.5">{topThree[2].xp.toLocaleString()} XP</span>
                        </div>
                ...
                <div>
                  <span className="text-[10px] font-extrabold text-zinc-500 uppercase tracking-widest block mb-4">League Rankings</span>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {leaderboard.map((entry) => {
                      ...
                      const favoriteHabit = entry.rank % 3 === 0 ? "🚲 Cycle Commute" : entry.rank % 3 === 1 ? "🥗 Plant Diet" : "⚡ Unplug Standby";
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="absolute top-4 right-4 flex items-center gap-1">
                            <span className="text-[10px] font-bold font-mono text-zinc-500">#{entry.rank}</span>
                            {getRankMovementIcon(entry.rank_movement)}
                          </div>

                          {/* Profile Avatar Username */}
                          <div className="flex items-center gap-3">
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-xs font-syne ${
                            ...
                            <div>
                              <div className="flex items-center gap-1.5">
                                <span className="text-xs font-bold text-white line-clamp-1">{entry.username}</span>
                                {isSelf && (
                                  <span className="text-[8px] font-extrabold px-1 py-0.5 bg-accent/20 border border-accent/30 text-accent rounded uppercase">You</span>
                                )}
                              </div>
                              <span className="text-[9px] text-zinc-500 font-mono mt-0.5 block">Level {level} • {entry.xp.toLocaleString()} XP</span>
                            </div>
                          </div>
                          ...
                          <div className="space-y-2">
                            <span className="text-[8px] text-zinc-500 uppercase font-extrabold tracking-widest block">Planet Status</span>
                            ...
                            <div className="w-full bg-zinc-900 h-1 rounded-full overflow-hidden border border-white/5">
                              <div className="h-full bg-accent" style={{ width: `${vegetationValue}%` }} />
                            </div>
                            ...
                            <div className="w-full bg-zinc-900 h-1 rounded-full overflow-hidden border border-white/5">
                              <div className="h-full bg-secondary" style={{ width: `${waterValue}%` }} />
                            </div>
                          </div>
            ...
            {activeTab === "groups" && (
              ...
              >
                ...
                <div className="flex items-center justify-between border-b border-white/5 pb-4 mb-4">
                  <div>
                    <h2 className="font-syne font-bold text-base text-white">Active Eco-Villages</h2>
                    <p className="text-xs text-zinc-400">Cooperate in villages to aggregate carbon offset goals together.</p>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6" id="groups-list-grid">
                  {groups.map((group) => {
                    ...
                    return (
                      ...
                      >
                        <div>
                          <span className={`inline-block px-2.5 py-0.5 rounded text-[8px] font-extrabold uppercase tracking-widest border ${getGroupTypeColor(group.type)}`}>
                            {villageType}
                          </span>
                          <h3 className="font-syne font-bold text-sm md:text-base text-zinc-100 mt-2.5">{group.name}</h3>
            ...
            {activeTab === "badges" && (
              ...
              >
                <div className="border-b border-white/5 pb-3">
                  <h2 className="font-syne font-bold text-base text-white">Ecology Collectible Achievements</h2>
                  <p className="text-xs text-zinc-400 mt-0.5">Collect milestones as glowing collectible cards. Higher rewards yield rare tiers.</p>
                </div>

                {/* Collectible Badge Grid */}
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-4 animate-show" id="badges-shelf-grid">
                  {achievements.map((ach) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className={`text-[8px] font-extrabold uppercase tracking-widest font-mono block self-end ${rarity.labelColor}`}>
                          {rarity.name}
                        </span>

                        {/* Large Emoji Badge */}
                        <div className={`w-16 h-16 rounded-full flex items-center justify-center text-3xl select-none transition-all ${
                          ach.unlocked
                        ...
                        {/* Card Details */}
                        <div>
                          <h4 className="font-syne font-extrabold text-sm text-zinc-200">{ach.name}</h4>
                        ...
                        <span className={`text-[9px] font-bold px-3 py-1 rounded-full uppercase tracking-wider font-mono border ${rarity.badgeGlow}`}>
